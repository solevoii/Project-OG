...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
	...
	protected internal string AppVersion => string.Format("{0}_{1}", PhotonNetwork.gameVersion, "1.86.1");
	...
	protected internal void SetupProtocol(ServerConnection serverType)
	{
		...
		if (base.TransportProtocol != connectionProtocol)
		{
			if (PhotonNetwork.logLevel >= PhotonLogLevel.ErrorsOnly)
			{
				UnityEngine.Debug.LogWarning("Protocol switch from: " + base.TransportProtocol + " to: " + connectionProtocol + ".");
			}
	...
	public virtual bool OpLeave()
	{
		...
		{
			UnityEngine.Debug.LogWarning("Not sending leave operation. State is not 'Joined': " + State);
			return false;
		}
	...
	public void OnStatusChanged(StatusCode statusCode)
	{
		...
		case StatusCode.Disconnect:
			...
			if (State == ClientState.DisconnectingFromGameserver || State == ClientState.DisconnectingFromNameServer)
			{
		...
		case StatusCode.TimeoutDisconnect:
			if (IsInitialConnect)
			{
				UnityEngine.Debug.LogWarning(statusCode + " while connecting to: " + base.ServerAddress + ". Check if the server is available.");
				IsInitialConnect = false;
				DisconnectCause disconnectCause = (DisconnectCause)statusCode;
				SendMonoMessage(PhotonNetworkingMessage.OnFailedToConnectToPhoton, disconnectCause);
			}
	...
	public void OnEvent(EventData photonEvent)
	{
		...
		case 209:
		{
			...
			if (PhotonNetwork.logLevel == PhotonLogLevel.Informational)
			{
				UnityEngine.Debug.Log("Ev OwnershipRequest " + photonView.ownershipTransfer + ". ActorNr: " + num + " takes from: " + num3 + ". local RequestedView.ownerId: " + photonView.ownerId + " isOwnerActive: " + photonView.isOwnerActive + ". MasterClient: " + mMasterClientId + ". This client's player: " + PhotonNetwork.player.ToStringFull());
			}
			...
			case OwnershipOption.Takeover:
				if (num3 == photonView.ownerId || (num3 == 0 && photonView.ownerId == mMasterClientId) || photonView.ownerId == 0)
				{
					...
					if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
					{
						UnityEngine.Debug.LogWarning(photonView + " ownership transfered to: " + num);
					}
	...
	protected internal void ExecuteRpc(ExitGames.Client.Photon.Hashtable rpcData, PhotonPlayer sender)
	{
		...
		if (photonView == null)
		{
			...
			if (flag)
			{
				UnityEngine.Debug.LogWarning("Received RPC \"" + text + "\" for viewID " + num + " but this PhotonView does not exist! View was/is ours." + ((!flag2) ? " Remote called." : " Owner called.") + " By: " + sender.ID);
			}
			else
			{
				UnityEngine.Debug.LogWarning("Received RPC \"" + text + "\" for viewID " + num + " but this PhotonView does not exist! Was remote PV." + ((!flag2) ? " Remote called." : " Owner called.") + " By: " + sender.ID + " Maybe GO was destroyed but RPC not cleaned up.");
			}
			return;
		}
	...
	public void CleanRpcBufferIfMine(PhotonView view)
	{
		...
		{
			UnityEngine.Debug.LogError("Cannot remove cached RPCs on a PhotonView thats not ours! " + view.owner + " scene: " + view.isSceneView);
		}
		else
		{
	...
	internal void RPC(PhotonView view, string methodName, PhotonTargets target, PhotonPlayer player, bool encrypt, params object[] parameters)
	{
		...
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
		{
			UnityEngine.Debug.Log("Sending RPC \"" + methodName + "\" to target: " + target + " or player:" + player + ".");
		}
		ExitGames.Client.Photon.Hashtable hashtable = new ExitGames.Client.Photon.Hashtable();
		hashtable[(byte)0] = view.viewID;
		if (view.prefix > 0)
		...
		case PhotonTargets.Others:
		{
			...
			raiseEventOptions.InterestGroup = view.group;
			raiseEventOptions.Encrypt = encrypt;
			RaiseEventOptions raiseEventOptions10 = raiseEventOptions;
			OpRaiseEvent(200, hashtable, sendReliable: true, raiseEventOptions10);
			break;
		}
		...
		case PhotonTargets.AllGlobalBuffered:
		{
			RaiseEventOptions raiseEventOptions = new RaiseEventOptions();
			raiseEventOptions.InterestGroup = view.group;
			raiseEventOptions.Receivers = ReceiverGroup.All;
