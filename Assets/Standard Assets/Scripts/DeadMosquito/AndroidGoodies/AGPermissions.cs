	...
	public static class AGPermissions
	{
		public class PermissionRequestResult
		{
			...
			public static PermissionRequestResult FromJson(Dictionary<string, object> serialized)
			{
				...
				permissionRequestResult.Permission = serialized.GetStr("permission");
				permissionRequestResult.ShouldShowRequestPermissionRationale = (bool)serialized["shouldShowRequestPermissionRationale"];
				permissionRequestResult.Status = (PermissionStatus)(long)serialized["result"];
				return permissionRequestResult;
			}

			public override string ToString()
			{
				return $"[PermissionRequestResult: Permission={Permission}, Status={Status}, ShouldShowRequestPermissionRationale={ShouldShowRequestPermissionRationale}]";
			}
		}
		...
		private static Action<PermissionRequestResult[]> _onRequestPermissionsResults;
		...
		public static bool IsPermissionGranted(string permission)
		{
			...
			try
			{
				using (AndroidJavaClass ajc = new AndroidJavaClass("android.support.v4.content.ContextCompat"))
				{
					return ajc.CallStaticInt("checkSelfPermission", AGUtils.Activity, permission) == 0;
				}
			}
			catch (Exception ex)
			{
				...
				{
					UnityEngine.Debug.LogWarning("Could not check if runtime permission is granted. Check if Android version is 6.0 (API level 23) or higher. " + ex.Message);
				}
				return false;
			}
		}
		...
		public static void RequestPermissions(string[] permissions, Action<PermissionRequestResult[]> onRequestPermissionsResults)
		{
			...
			if (!AGUtils.IsNotAndroidCheck() && !IsPriorToMarshmellow())
			{
				try
				{
				...
				catch (Exception ex)
				{
					if (Debug.isDebugBuild)
					{
						UnityEngine.Debug.LogWarning("Could not request runtime permissions. " + ex.Message);
					}
				}
			}
		}

		private static bool IsPriorToMarshmellow()
		{
			bool flag = AGDeviceInfo.SDK_INT < 23;
			if (flag)
			{
				UnityEngine.Debug.Log("Runtime permissions on Android below version 23 are not required and this method will do nothing");
			}
			return flag;
		}
		...
		public static void OnRequestPermissionsResult(string message)
		{
			if (_onRequestPermissionsResults != null)
			{
				...
				PermissionRequestResult[] array = new PermissionRequestResult[list.Count];
				for (int i = 0; i < list.Count; i++)
				{
					PermissionRequestResult permissionRequestResult = array[i] = PermissionRequestResult.FromJson(list[i] as Dictionary<string, object>);
				}
				_onRequestPermissionsResults(array);
